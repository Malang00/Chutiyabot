...
async def txt_handler(bot: Client, m: Message):
    ...
    try:
        ...
        for i in range(count-1, len(links)):
            ...
            url = urly.replace('"', '').replace(',', '').replace('(','').replace(')','').replace("d1d34p8vz63oiq", "d26g5bnklkwsh4").replace("d31cfla5yh9cc4.cloudfront.net", "appxcontent-mcdn.akamai.net.in").replace("pw2.pc.cdn.bitgravity.com","d26g5bnklkwsh4.cloudfront.net").replace("file/d/","uc?export=download&id=").replace("www.youtube-nocookie.com/embed", "youtu.be").replace("?modestbranding=1", "").replace("/view?usp=sharing","").replace("d3nzo6itypaz07", "d26g5bnklkwsh4").replace("iframe.mediadelivery.net/embed/10984-", "b7ql8e5g.fs-vid.com/").replace("?autoplay=true&preload=false", "/playlist.m3u8").replace("dn6x93wafba93", "d26g5bnklkwsh4").replace("d2tiz86clzieqa", "d26g5bnklkwsh4").replace("vod.teachx.in", "d3igdi2k1ohuql.cloudfront.net").replace("downloadappx.appx.co.in", "d33g7sdvsfd029.cloudfront.net").strip()
            parsed_url = urlparse(url)
